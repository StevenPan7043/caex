...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 周期交易 <span class="c-gray en">&gt;</span> 跟单交易列表 <a
	...
	<form action="/cycle/showRecordList" id="seachFrom">
				...
				<tbody>
					<c:forEach items="${pageInfo.list }" var="l">
						<tr >
							...
							<td class="tc">${l.num}</td>
							<td class="tc">${l.outputCurrency}</td>
							<td class="tc">${l.price}</td>
							<td class="tc">${l.total}</td>
							<td class="tc">
								<c:if test="${l.status==1}">有效</c:if>
								<c:if test="${l.status==2}">无效</c:if>
							</td>
							<td class="tc"><fmt:formatDate value="${l.createTime}" pattern="yyyy-MM-dd HH:mm" /></td>
							<td class="tc"><fmt:formatDate value="${l.modifyTime}" pattern="yyyy-MM-dd HH:mm" /></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
