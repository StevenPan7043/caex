...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 合约交易 <span class="c-gray en">&gt;</span> 逐仓订单列表 <a
	...
	<form action="/order/showZcList" id="seachFrom">
		<div class="page-container">
			<div class="col-12"> 
				...
				<div class="col-4 cl f-l">
					...
					<select name="status" class="input-text "style="width:120px;">
							...
							<option value="1" <c:if test="${order.status==1}">selected="selected"</c:if>>逐仓中</option>
			...
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tbody>
					<c:forEach items="${pageInfo.list }" var="l">
						<tr >
							...
							<td class="tc">${l.capital }</td>
							<td class="tc">${l.tax }</td>
							<td class="tc">
								<p>持仓价:${l.buyprice }</p>
								<c:if test="${l.status==2 }"><p>平仓价:${l.stopprice }</p></c:if>
							</td>
							<td class="tc">${l.reward }</td>
							<td class="tc">
								<p>止盈:${l.stopwin }</p>
								<p>止损:${l.stopfail }</p>
							</td>
				...
				<tfoot>
					<tr>
						<th colspan="14">总保证金:<fmt:formatNumber value="${map.money }" pattern="#0.000000" />  
						/ 总手续费:${map.tax }  / 总盈亏:<fmt:formatNumber value="${map.rates }" pattern="#0.000000" />  / 逐仓总余额:<fmt:formatNumber value="${map.balance }" pattern="#0.000000" /> </th>
					</tr>
				</tfoot>
			</table>
		</div>
