...
import com.pmzhongguo.market.exception.BusinessException;
...
public class JedisUtil {
	...
	public void zadd(byte[] keyName, double score, byte[] obj) {
		...
		if (shardedJedis == null || null == obj || null == keyName) {
			throw new BusinessException(-1, new Exception().getMessage());
		}
		try {
...
//			Protocol.DEFAULT_TIMEOUT, HelpUtils.getMgrConfig().getRedis_pwd());
			pool = new JedisPool(config, InitConstant.REDIS_HOST, Integer.parseInt(InitConstant.REDIS_PORT),
					10 * 1000, InitConstant.REDIS_PASSWORD);
		}
		return pool;
	}
	...
	public boolean exists(byte[] keyName) {
		...
		try {
			return shardedJedis.exists(keyName);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	 ...
	 * @return
	 */
	public boolean keys(byte[] keyName, Map<String, Set<byte[]>> map) {
		Jedis shardedJedis = getJedis();
		if (shardedJedis == null || null == keyName) {
			return false;
		}
		try {
			Set<byte[]> keys = shardedJedis.keys(keyName);
			Object deserialize = SerializeUtil.deserialize(keys.iterator().next());
			if (keys.size() == 0) {
				return false;
			}
			map.put("keys", keys);
