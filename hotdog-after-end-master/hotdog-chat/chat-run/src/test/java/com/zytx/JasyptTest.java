...
public class JasyptTest {
     ...
     * 1、Jasypt 默认使用 来解密全局配置文件中的属性，所以提供密文时，也需要提供加密的密文
     * 2、接口有很多的实现类，比如常用的 {@link PooledPBEStringEncryptor}
     * 3、setConfig(final PBEConfig config)：为对象设置配置对象
    ...
    /**
     * 设置配置对象，SimpleStringPBEConfig 是它的实现类
    ...
    public static void main(String[] args) throws Exception {
        PooledPBEStringEncryptor pooledPBEStringEncryptor = new PooledPBEStringEncryptor();
        SimpleStringPBEConfig config = new SimpleStringPBEConfig();
        config.setPassword("zytx");
        config.setAlgorithm("PBEWithMD5AndDES");
        config.setPoolSize("1");
        pooledPBEStringEncryptor.setConfig(config);
        String encrypt = pooledPBEStringEncryptor.encrypt("123456");
        System.out.println(encrypt);
        String decrypt = pooledPBEStringEncryptor.decrypt(encrypt);
