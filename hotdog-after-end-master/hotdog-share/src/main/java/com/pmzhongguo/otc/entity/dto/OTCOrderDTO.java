...
public class OTCOrderDTO implements Serializable
{
    ...
    //剩余交易额。限价卖单：remain_volume*price 限价买单：remain_volume
    ...
    //锁定交易额，对限价卖单，同已锁定数量，对限价买单，等于『成交』价格*数量。对市价单等于已成交数量
    ...
    public BigDecimal getCurAmount()
    {
        try
        {
            if (OTCOrderUtil.isLimitBuy(this))
            {
                curAmount = this.getCurVolume() == null || this.getPrice() == null ? null : this.getPrice().multiply(this.getCurVolume());
            } else
            {
                curAmount = this.getCurVolume();
            }
        } catch (Exception e)
        {
            curAmount = null;
...
//	}
    ...
    public WhetherEnum getIsAds()
    {
        return isAds;
    }

    public void setIsAds(WhetherEnum isAds)
    {
        this.isAds = isAds;
    }
