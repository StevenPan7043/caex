(function($) {
	...
	function display(index, trigger, e, options) {
		...
		//周建伟：调整一下显示位置，防止在页面底部的时候，被隐藏掉
		var adjustLeft = e.pageX, adjustTop = e.pageY;
		if ((adjustLeft + menu.width()) > $(window).width()) {
			adjustLeft = adjustLeft - ((adjustLeft + menu.width()) - $(window).width());
		}
		if ((adjustTop + menu.height()) > $(window).height()) {
			adjustTop = adjustTop - ((adjustTop + menu.height()) - $(window).height());
		}
