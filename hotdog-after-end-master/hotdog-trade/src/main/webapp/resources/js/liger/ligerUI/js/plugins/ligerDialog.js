    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            if (!p.fixedType)
            {
				...
				var height = p.height || g.dialog.height();
		...
		//var screenWidth = document.body.clientWidth, screenHeight = document.body.clientHeight; //当前浏览器窗口的宽高
		var screenWidth = $(window).width(), screenHeight = $(window).height(); //当前浏览器窗口的宽高
		//alert(screenWidth + "," + screenHeight);
		var scrolltop = $(document).scrollTop();//获取当前窗口距离页面顶部高度
		//alert(scrolltop);
		var objLeft = (screenWidth - width)/2;
		var objTop = (screenHeight - height)/2 + scrolltop;
		//alert(objTop);
		g.dialog.css({left: objLeft + 'px', top: objTop + 'px'}); 
		
		*/

/*
		var closeObjLeft = (screenWidth + width)/2-closeObj.width()-2;
		var closeObjTop = (screenHeight - height)/2 + scrolltop+12;
		
		g.dialog.css({left: closeObjLeft + 'px', top: closeObjTop + 'px'});
		*/
...
/*
		...
		$(window).resize(function() {
			...
			objLeft = (screenWidth - width)/2;
			objTop = (screenHeight - height)/2 + scrolltop;
			g.dialog.css({left: objLeft + 'px', top: objTop + 'px'});
			var closeObjLeft = (screenWidth + width)/2-closeObj.width()-2;
			var closeObjTop = (screenHeight - height)/2 + scrolltop+12;
			g.dialog.css({left: closeObjLeft + 'px', top: closeObjTop + 'px'});
