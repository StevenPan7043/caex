    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if (p.contextmenu && $.ligerMenu)
            {
                g.tab.menu = $.ligerMenu({ width: 100, items: [
                    ...
                    /*{ text: p.addToFolder, id: 'addtofolder', click: function ()
                    {
                        g._menuItemClick.apply(g, arguments);
                    }
                    },*/
         ...
         addtofolder: function (tabid)
        {
           ...
           // var iframeloading = $(".l-tab-loading:first", contentitem);
            ...
            //var url = $(iframe).attr("src");
            //iframeloading.show();
            setAddToFolder(tabid);
			//var subDialog = $.ligerDialog.open({title:"添加快捷方式", url:'/toAddToFolder?func_id='+tabid ,width : 435,height: 245,isResize: false,isHidden : false});
            /*iframe.attr("src", url).unbind('load.tab').bind('load.tab', function ()
            {
                iframeloading.hide();
            });*/
        ...
        _menuItemClick: function (item)
        {
            ...
            switch (item.id)
            {
                ...
                case "closeall":
                    ...
                    break;
                case "addtofolder":
                    if (g.trigger('addtofolder', [{ tabid: g.actionTabid }]) == false) return;
                    g.selectTabItem(g.actionTabid);
                    g.addtofolder(g.actionTabid);
                case "reload":
        ...
        _addTabItemContextMenuEven: function (tabitem)
        {
            ...
            tabitem.bind("contextmenu", function (e)
            {
                ...
                if ($(".l-tab-links-item-close", this).length == 0)
                {
                    ...
                    g.tab.menu.setDisabled('addtofolder');
                }
                else
                {
                    g.tab.menu.setEnabled('close');
                    g.tab.menu.setEnabled('addtofolder');
                }
                return false;
            });
        }
