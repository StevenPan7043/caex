...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 15, 30, 40, 50, 100],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
    		...
    		if (p.autoRefresh) {
    			var autoRefreshHtml = $('<a class="a_autoRefresh a_reshStart" href="#" title="自动刷新">开启自动刷新</a>');
			...
			if (p.excel) {
				var exportHtml = $('<a class="a_exportExcel fnExport" href="#" title="导出为Excel">导出为Excel</a>');
	 			if(typeof(p.excel)=='string'){
	 				p.excel={url:p.excel};
	 			}
	 			$.applyIf(p.excel,{displays:[],names:[]});
	 			$.each(p.columns,function(n,v){
	 				if(v.display&&v.name&&v.isHide!==false&&v.name!='_rownum'){
	 			...
	 			//$('.fnExport',g.toolbar).data("load-param",param);
	 		}
        },
        ...
        loadData: function (loadDataParm)
        {
            ...
            //Kevin:add
            if(p.excel)$('.fnExport',g.toolbar).data("load-param",param);
            //
            g.loading = false;
        },
