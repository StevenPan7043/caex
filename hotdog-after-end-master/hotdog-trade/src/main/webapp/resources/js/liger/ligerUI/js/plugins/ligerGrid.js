...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [5, 10, 15, 20, 30, 40, 50, 100, 200, 500, 1000],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
    	...
    	excel:false,                     //导出为Excel
		autoRefresh:false,				//开启自动刷新
        ...
        heightDiff: -15,                         //高度补差,当设置height:100%时，可能会有高度的误差，可以通过这个属性调整
        cssClass: null,                    //类名
        root: 'Rows',                       //数据源字段名
        ...
        whenRClickToSelect: true,                //右击行时是否选中
        contentType: 'application/json',                     //Ajax contentType参数
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '从{from}到{to},共{total}条.',
    ...
    $.ligerDefaults.Grid_columns = {
        id: null,
        name: null,
        sortColumnName: null, //专门用于排序的（By周建伟）
        totalSummary: null,
        display: null,
        headerRender: null,
        isAllowHide: true,
        ...
        align: 'left',
        hide: false,
        excelExport: false,  //这个定义有点别扭，用于解决上一个字段（hide）为true，但又要导出的情况
        editor: null,
        render: null,
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
			...
			if (p.autoRefresh) {
				var autoRefreshHtml = $('<a class="a_autoRefresh a_refreshStart" href="javascript:void(0);" title="自动刷新">开启自动刷新</a>');
				$('.l-bar-group:last',g.toolbar).append(autoRefreshHtml);
				var resh=function(){g.loadData();};
				var tm=30000;
				var intervalId=setInterval(resh,tm);
				autoRefreshHtml.click(function () {
					$(this).toggleClass('a_refreshStart');
					if(p.onReload && p.onReload(grid) == false) return false;
						if (p.resource != 'project' && g.isDataChanged && !confirm(p.isContinueByDataChanged))
							return false;
						if($(this).hasClass('a_refreshStart')&&!intervalId){
							intervalId= setInterval(resh,tm);
						}else{
			...
			if (p.excel) {
				var exportHtml = $('<a class="a_exportExcel exportExcel" href="javascript:void(0);" title="导出为Excel">导出为Excel</a>');
				if(typeof(p.excel)=='string'){
					p.excel={url:p.excel};
				}
				$.applyIf(p.excel,{displays:[],names:[]});
				$.each(p.columns,function(n,v){
					if(v.display&&v.name&&v.isHide!==false&&(v.hide!=true||v.excelExport==true)&&v.name!='_rownum'){
						p.excel.displays.push(v.display.replace(/<\/?.+?>/g,""));
						p.excel.names.push(v.name);
					}
				});
				exportHtml.data("excel",p.excel);
				$('.l-bar-group:last',g.toolbar).append(exportHtml);
        	}
        },
        _reExcel: function ()
        {
        	var g = this, p = this.options;
			if (p.excel) {
				var exportHtml = $('.a_exportExcel');
				if(typeof(p.excel)=='string'){
					p.excel={url:p.excel};
				}
				$.applyIf(p.excel,{displays:[],names:[]});
				$.each(p.columns,function(n,v){
					if(v.display&&v.name&&v.isHide!==false&&v.hide!=true&&v.name!='_rownum'){
						p.excel.displays.push(v.display);
						p.excel.names.push(v.name);
					}
				});
				exportHtml.data("excel",p.excel);
        ...
        //刷新数据
        loadData: function (loadDataParm, needToTop)
        {
        	// 记录滚动条的位置
			var yPos = $(".l-grid-body2").scrollTop();
            ...
            //周建伟添加
            if(p.excel)$('.exportExcel',g.toolbar).data("load-param",param);
            //
            g.loading = false;
            
            if (needToTop == undefined) {
	            $(".l-grid-body2").animate({
	                scrollTop: yPos
	            });
            }
        },
        ...
        */
        ...
        changePage: function (ctype)
        {
            var g = this, p = this.options;
            if (g.loading) return true;
            if (p.resource != 'project' && p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                return false;
            ...
            if (p.usePager)
            {
                ...
                if (p.switchPageSizeApplyComboBox && $.fn.ligerComboBox)
                {
                    $(".l-bar-selectpagesize select", g.toolbar).ligerComboBox(
                    {
                        ...
                        {
                            if (p.resource != 'project' && p.url && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                                return false;
                            return true;
                        },
                        width: 45
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            var htmlarr = [];
             if(column.editor){
            	 htmlarr.push('<div class="l-grid-row-cell-inner x-editor"');
            }else{
            ...
            var tipContent = '';
            if (column.isTip) {
            	if (!column.tipName || column.tipName == '') {
            		tipContent = content;
            	} else {
            		tipContent = g._getValueByName(rowdata, column.tipName);
            	}
            	htmlarr.push(' title="' + tipContent + '" ');
            }
            ...
            //是否变色
            var rowTextColor = g._getValueByName(rowdata, "rowTextColor");
            if (rowTextColor) {
            	htmlarr.push(' color:' + rowTextColor + '; ');
            }
        ...
        _setEvent: function ()
        {
            ...
            {
                if (p.resource != 'project' && g.isDataChanged && p.dataAction != "local" && !confirm(p.isContinueByDataChanged))
                    return false;
                p.newPage = 1;
        ...
        _onClick: function (e)
        {
            ...
            else if (src.hcelltext) //排序
            {
                ...
                if (p.resource != 'project' && p.url && p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged)) return;
                var sort = $(".l-grid-hd-cell-sort:first", hcell);
                var columnName = src.column.sortColumnName || src.column.name;
