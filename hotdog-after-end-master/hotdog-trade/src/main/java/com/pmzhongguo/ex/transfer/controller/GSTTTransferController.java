 ...
 * @description：GSTT划转controller
 * @version: $
 */
@CrossOrigin(origins = "*", maxAge = 3600)
@Controller
@RequestMapping("/api/" + GSTTTransferController.c_name)
@ApiIgnore
public class GSTTTransferController extends TopController {
    @Autowired
    ...
    public static final String c_name = "GSTTProject";
    ...
    public ObjResp getUserSSOToken(HttpServletRequest request) {
        ...
        RespUserSecret reqBaseSecret = sSOUsersTokenManager.getUserSSOToken(member, c_name);
    ...
    public Resp fundTransferDeposit(@RequestBody ApiFundTransfer apiFundTransfer, HttpServletRequest request) {
        ...
        try {
            resp = fundTransferLogManager.addFundTransferLog(apiFundTransfer, member, c_name, ipAddr, OptSourceEnum.GSTTPROJECT);
        } catch (Exception e) {
            // 异常
            logger.warn("<=============== gstt 划转提现 异常：【{}】",e.getMessage());
            e.printStackTrace();
