...
public class JedisUtilMember {
    ...
    public void setMember(HttpServletRequest request, Member member) {
        ...
        if (!HelpUtils.nullOrBlank(token)) {
            ...
            try {
                if (request != null) {
                    String loginToken = request.getHeader(Constants.LOGIN_TOKEN);

                    if (StringUtil.isNullOrBank(loginToken)) {
                        return;
                    }
                    if (loginToken.equals("app") || loginToken.equals("web")) {
                        token = loginToken + "_" + token;
                    }
                }
     ...
     * @return
     */
    public Member getMember(HttpServletRequest request, String token) {
        Member m = null;
        if (request != null) {
            String loginToken = request.getHeader(Constants.LOGIN_TOKEN);
            if (!StringUtil.isNullOrBank(loginToken)) {
        ...
        // token 方式
        if (!HelpUtils.nullOrBlank(token) && !"app".equals(token) && !"web".equals(token)) {
            Jedis shardedJedis = getJedis();
            if (shardedJedis == null) {
                return null;
            }
            ...
            // session方式
            HttpSession session = request.getSession();
            m = (Member) session.getAttribute(Constants.SYS_SESSION_MEMBER);
        }
        return m;
    }
