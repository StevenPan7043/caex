...
@Controller
@RequestMapping("/api/" + LastWinnerTransferController.c_name)
@ApiIgnore
public class LastWinnerTransferController extends TopController
{
    ...
    public static final String c_name = "lastWinner";
    ...
    public Resp fundTransferDeposit(@RequestBody ApiFundTransfer apiFundTransfer, HttpServletRequest request) {
        ...
        try {
            ...
            resp = fundTransferLogManager.addFundTransferLog(apiFundTransfer, member, c_name, ipAddr, OptSourceEnum.LASTWINNER);
        } catch (Exception e) {
            // 异常
    ...
    public Resp fundTransferWithdraw(@RequestBody ApiFundTransfer apiFundTransfer, HttpServletRequest request) {
        ...
        try {
            ...
            // 异常
            logger.warn("<=============== lastwinner 划转提现 异常：【{}】",e.getMessage());
            e.printStackTrace();
    ...
    public RespObj commonTransfer(ApiFundTransfer apiFundTransfer, String c_name, HttpServletRequest request)
    {
        ...
        try {
            ...
            mapParam.put("tradeID",apiFundTransfer.getTradeID());
            mapParam.put("api_key",apiFundTransfer.getApi_key());
            mapParam.put("sign_type",apiFundTransfer.getSign_type());
            mapParam.put("timestamp",apiFundTransfer.getTimestamp()+"");

            Map<String, Object> _toSignMap = new HashMap<>();
            _toSignMap.put("tradeID",apiFundTransfer.getTradeID());
            _toSignMap.put("api_key",apiFundTransfer.getApi_key());
            _toSignMap.put("sign_type",apiFundTransfer.getSign_type());
            _toSignMap.put("timestamp",apiFundTransfer.getTimestamp());

            logger.warn("划转验证请求参数是否合法,回调函数url：【{}】,参数：{}",url,new Gson().toJson(apiFundTransfer));
            mapParam.put("sign",HelpUtils.createSign(_toSignMap,thirdPartyInfo.getS_secretKey()));
            String result = HttpUtils.post(url, mapParam);
