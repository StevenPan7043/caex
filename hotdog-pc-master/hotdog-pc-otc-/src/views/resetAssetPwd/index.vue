<template>
  <div class="eidtPwd_container">
    <div class="title_container">
      <p><router-link to="/user">个人中心</router-link></p>><span>设置资金密码</span>
    </div>
    <div class="edit_container">
      <Form :model="formTop" label-position="top" class="form_container">
        <FormItem label="资金密码">
          <Input v-model="formTop.pwd" type="password" @on-blur="pwdCheck"></Input>
        </FormItem>
        <FormItem label="确认资金密码">
          <Input v-model="formTop.confirmPwd" type="password"></Input>
        </FormItem>
...
import { getKaptcha, setSecPwd } from './api';
...
export default {
  ...
  methods: {
    ...
    submitClick() {
      if (this.formTop.pwd !== this.formTop.confirmPwd) {
        this.$Message.error({
          content: '两次密码不一致',
          duration: 2,
          closable: true,
      ...
      setSecPwd(params).then(({ state, data, msg }) => {
        if (state === 1) {
