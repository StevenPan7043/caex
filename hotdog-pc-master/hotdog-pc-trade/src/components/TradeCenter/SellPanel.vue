    ...
    <div class="inner-panel">
      ...
      <div class="buy-in-price price">
        ...
        <div class="input-wrap">
          <input type="text" maxlength="14" v-model="buyinPrice" @input="buyinPriceChange"/>
  ...
  methods: {
    async getLimit () {
      ...
      if (res.data[this.symbol]  === null) {
        ...
        const limitData = res.data[this.symbol].lowPrice
    ...
    buyinPriceChange () {
      ...
      if (inputVal && this.state) {
        ...
        this.buyMaxNum = (Math.floor(parseFloat(balance) * pow) / pow).toFixed(
    ...
    async buyinBtnClick () {
      ...
      if (!isLimit) {
        this.$message({
          type: 'error',
          message: `低于当日最大跌幅限价：${this.limitPrice}`,
          duration: 3000,
          showClose: true
        })
...
<style>
  .sell-out .el-slider__bar {
    background: #da3f4d !important;
  }
  ...
  .sell-out .el-slider__button {
    border: 3px solid #da3f4d;
    background: #da3f4d;
    width: 10px;
    height: 10px;
  }

  .sell-out .el-slider__stop {
