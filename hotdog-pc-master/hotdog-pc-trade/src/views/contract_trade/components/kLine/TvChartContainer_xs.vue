  ...
  methods: {
    ...
    TVChartInit() {
      ...
      Datafeeds.WebsockFeed.prototype.getBars = function (
        ...
        // let startDate = Math.round(curTime - 60 * 3600 * 24)
        // // 根据时间周期来加载
        // if (firstDataRequest) {
        //   mythat
        //     .$get(
      ...
      Datafeeds.WebsockFeed.prototype.subscribeBars = function (
        ...
        function subscribeK () {
          resolution = mythat.getResolutionFormat(resolution)
          mythat.$ajax(`/account/getKlineCurrent?token=${mythat.$userID()}&symbol=${mythat.symbol.toLowerCase()}&klineType=${resolution}`).then(res => {
